...
## Phase 2: Prefix-Only Grammar + Enhanced Mutator ~~(Week 2-3)~~ DONE

### Files to modify
- [x] `generator/src/grammar.rs` -- Restructure as PREFIX_GRAMMAR
- [x] `generator/src/main.rs` -- `--prefix-only` flag (default true), 7 golden suffixes, multi-suffix testing
- [x] `generator/src/bin/stress.rs` -- `--prefix-only` flag (default true)

### Grammar restructuring
1. [x] **Remove**: GOLDEN_CHECK, GOLDEN_THEOREM, GOLDEN_VERIFY, PROOF_TERM, TACTIC, TACTIC_SEQ, CASE_ARMS, RCASES_PAT (these are in the fixed suffix now)
2. [x] **Keep**: All declaration-level non-terminals (INDUCTIVE_DECL, DEF_DECL, etc.) -- these already exist in the 515-rule grammar
3. [x] **Expand**: Add many more rules for the 7 attack categories above (502→613 full rules, 393→504 prefix rules)
4. [x] **Add**: THEOREM_DECL without proof (e.g., `theorem helper : P := sorry` -- these sorrys in *helper lemmas* are fine as long as the golden suffix doesn't use them. Actually: **prefix declarations CAN use sorry** in non-False theorems to build up a library of "proved" lemmas that the golden suffix's automation might use)
...
1. [x] **ATTR_DECL**: `@[simp] theorem ...`, `@[instance] def ...` (12 rules)
2. [x] **NOTATION_DECL**: Custom notation (8 rules)
3. [x] **SCOPED_DECL**: `scoped instance ...`, `local instance ...`, `attribute [...]` (5 rules + ATTR_NAME helper)
4. [x] **DERIVE_DECL**: `deriving instance Decidable` (2 rules + DERIVE_CLASS helper)
5. [x] **OPAQUE_TRICK**: `opaque` + `@[reducible]` combinations (expanded OPAQUE_DECL)
6. [x] **OPTION_CMD**: `set_option ... in` (10 rules)

### Integration: Wire generator output into scaffold pipeline
- [x] `gen-sample` outputs prefix-only code (no golden suffix, no proof terms)
- [x] Scaffold `fuzz` command consumes prefix-only output from generator
- [x] Shared contract: generator stdout = raw prefix, scaffold appends golden suffixes
...
- [x] Rust/LibAFL Nautilus for high-throughput grammar mutation (613 full / 504 prefix rules, 3 mutator types)
...
|-----------|---------|----------|--------|
| `generator/src/grammar.rs` | 515 rules including proofs/tactics | 613 full / 504 prefix rules with 7 new nonterminals | **Done** |
| `generator/src/main.rs` | Sync build, binary oracle, generates full files | `--prefix-only`, 7 golden suffixes, multi-suffix testing | **Done** |
...
| Golden check | Generated by grammar AND fixed suffix (disconnected) | FIXED suffix appended by assembler only | **Done** (both assembler and generator use 7 golden suffixes) |
...
| **Phase 2** | Prefix-Only Grammar + Enhanced Mutator | **DONE** |
...
**Key architectural gap (resolved in Phase 2):** The generator (Rust) now uses prefix-only grammar by default (`--prefix-only`) and tests all 7 golden suffixes per prefix. The `gen-sample` binary generates prefix-only code that the scaffold's assembler can consume directly. The `main` fuzzer and `stress` tester both support `--prefix-only` mode. Remaining gap: the scaffold's `fuzz` command and the Nautilus main fuzzer still run independently (no async queue between them yet — planned for Phase 4+).
